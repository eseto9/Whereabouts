...
function updateRemotes(dt,ts){
  ...
  for(const p of players()){
    ...
    if(!r){r={bean:prop?makeProp(prop):makeBean(safeCol(pr.col),safeFit(pr.fit),clean(pr.name)||'Bean'),sig,face:+pr.ry||0,tk:pr.tk,tkAt:0};r.bean.position.set(pr.x,+pr.y||0,+pr.z||0);Remote.set(p.peer,r);}
    const b=r.bean;const k=damp(12,dt);
    b.position.x=lerp(b.position.x,pr.x,k);b.position.y=lerp(b.position.y,(+pr.y||0)+(pr.hb&&!prop?rideLift(pr.fit):0),k);b.position.z=lerp(b.position.z,+pr.z||0,k);
    r.face=angLerp(r.face,+pr.ry||0,k);b.rotation.y=r.face;
    // a trick starts when we hear about it (their clock may not match ours)
    if(pr.tk!==r.tk){r.tk=pr.tk;r.tkAt=Date.now();}
    if(b.userData.body){animBean(b,{an:pr.an|0,em:typeof pr.em==='string'?pr.em:'',emAt:+pr.emAt||0,hb:!!pr.hb,tk:r.tkAt},ts);}
  }
