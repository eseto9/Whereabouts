...
function buildFarm(){
  ...
  {const g=new THREE.Group();const x0=29,x1=49,z0=15,z1=29;const rail='#E8D5B5';
   ...
   side(x0,z0,x0,z1-4);side(x0,z1,x1,z1);side(x1,z0,x1,z1);side(x0,z0,x0+6,z0);side(x0+10,z0,x1,z0);
   g.position.y=landH(39,22);scene.add(g);F(g,{k:'fence',n:'a fence',c:['cream'],s:'long',m:'wood',p:false});}
  {const t=signBox('Sunny Side Farm',3.4,0.8,'#FFD23F','#2B2040');const g=G_(at(box(0.16,1.8,0.16,'#8B5E3C'),-1.4,0.9,0),at(box(0.16,1.8,0.16,'#8B5E3C'),1.4,0.9,0));t.position.y=1.9;g.add(t);
   place(g,34,14.2,0.6,{k:'sign',n:'the farm sign',c:['yellow'],s:'flat',m:'wood',v:'sunny and friendly',u:'find the farm with',e:'🪧🌻🚜',r:['I stand at the gate and tell you the farm’s sunny name.']},{exact:true,ry:Math.PI});}
  ...
  {const g=new THREE.Group();
   ...
   barn=place(g,40,23,3.3,{k:'barn',n:'the red barn',c:['red','white'],s:'boxy',m:'wood',v:'cozy and a bit smelly',snd:'moo (from inside)',u:'store hay in',e:'🐄🏚️🌾',
  ...
  {const v=new THREE.Group();v.add(at(cyl(0.04,0.04,1.2,'#2B2040',5,{ol:false}),0,0.6,0));
   ...
   v.position.set(40,landH(40,23)+7,23);scene.add(v);
  ...
  {const h=mk(new THREE.TorusGeometry(0.22,0.05,5,10,Math.PI*1.4),'#B8B8C8',{w:0.015});h.rotation.set(0,Math.PI,Math.PI*0.8);
   h.position.set(40,landH(40,23)+3.3,23-2.72);scene.add(h);
  ...
  {const g=G_(at(cyl(1.3,1.3,7,'#A8B8C8',14),0,3.5,0),at(sph(1.32,'#8FA3B8',14,8),0,7,0));
   ...
   place(g,45.5,25,1.4,{k:'silo',n:'the grain silo',c:['silver','blue'],s:'tall and round',m:'metal',v:'standing to attention',u:'store grain in',e:'🌾🗼🥫',
     r:['I’m a giant tin can full of grain, standing next to the barn.']},{exact:true});}
  // farmhouse
  house(32.5,25.5,'#FFFFFF','#3E9E4A',{k:'farmhouse',n:'the farmhouse',c:['white','green'],s:'boxy',m:'wood',v:'homely',snd:'creak',u:'have breakfast in',e:'🏡🥞🐓',
  ...
  {const b=G_(at(box(0.22,0.35,0.35,'#FFD23F',{w:0.015}),-0.14,0.17,0),at(box(0.22,0.35,0.35,'#FFD23F',{w:0.015}),0.14,0.17,0));
   place(b,30.4,22.4,0.3,{k:'boots',n:'the rubber boots',c:['yellow'],s:'tall',m:'rubber',v:'ready for puddles',u:'stomp through mud in',e:'👢🌧️🐷',r:['Two of us wait by the door for a muddy day.']},{solid:false});}
  ...
  {const g=new THREE.Group();
   ...
   place(g,36,18.5,1.4,{k:'tractor',n:'the green tractor',c:['green','yellow'],s:'chunky',m:'metal',v:'hardworking',snd:'putt-putt',u:'plough a field with',e:'🚜🌱💪',
  ...
  {const g=new THREE.Group();const hay='#E8C45A';
   ...
   place(g,31.5,17.5,1.4,{k:'hay bales',n:'the hay bales',c:['yellow','gold'],s:'round',m:'hay',v:'a bit scratchy',u:'feed the cows with',e:'🌾🧻🐄',r:['I’m dried grass rolled up tight, waiting for winter.']},{exact:true});}
  // wishing well
  {const g=new THREE.Group();g.add(at(cyl(0.85,0.9,0.9,'#B8B2C8',12),0,0.45,0));g.add(at(mk(new THREE.CylinderGeometry(0.7,0.7,0.05,12),'#3B6E8C',{ol:false}),0,0.82,0));
   ...
   place(g,46.5,20,1,{k:'well',n:'the wishing well',c:['grey','red'],s:'round',m:'stone',v:'full of wishes',snd:'plink',u:'drop a coin into',e:'🪣🌟🪙',r:['Drop a coin into me and make a wish, but don’t fall in!']},{exact:true});}
  // chicken coop
  {const g=new THREE.Group();for(const sx of[-0.7,0.7])for(const sz of[-0.5,0.5])g.add(at(box(0.1,0.7,0.1,'#8B5E3C',{ol:false}),sx,0.35,sz));
   ...
   place(g,43,17.5,1.2,{k:'coop',n:'the chicken coop',c:['tan','red'],s:'boxy',m:'wood',v:'full of gossip',snd:'cluck-cluck',u:'collect eggs from',e:'🐔🏠🥚',r:['Hens sleep in me and leave surprises behind in the morning.']},{exact:true});}
  // windpump
  {const g=new THREE.Group();for(const [sx,sz] of[[-1,-1],[1,-1],[-1,1],[1,1]])g.add(rot(at(cyl(0.05,0.05,6,'#8FA3B8',4,{ol:false}),sx*0.45,3,sz*0.45),sz*0.08,0,-sx*0.08));
   ...
   place(g,47,26,0.8,{k:'windpump',n:'the windpump',c:['silver','red'],s:'tall',m:'metal',v:'spinning its heart out',snd:'clank-clank',u:'pump water with',e:'🌬️💧🌀',r:['My metal flower spins in the wind to pull water up from the ground.']},{exact:true});
  ...
  {const g=new THREE.Group();const glass=new THREE.MeshToonMaterial({color:'#CFF4FF',transparent:true,opacity:0.45,gradientMap:gradTex,depthWrite:false});
   ...
   place(g,36,26.5,2,{k:'greenhouse',n:'the greenhouse',c:['clear','green'],s:'boxy',m:'glass',v:'warm and steamy',u:'grow tomatoes in',e:'🍅🏠🌡️',r:['I’m a little glass house where tomatoes stay warm all winter.']},{exact:true});}
  ...
  {const g=G_(at(box(2,0.6,0.7,'#8FA3B8'),0,0.3,0),at(mk(new THREE.BoxGeometry(1.8,0.05,0.5),'#5FB6D6',{ol:false}),0,0.58,0));
   place(g,38,16.5,1,{k:'trough',n:'the water trough',c:['grey','blue'],s:'long',m:'metal',v:'always refreshing',snd:'slurp',u:'give the animals a drink from',e:'🐄💧🥤',r:['Thirsty animals drink from my long metal tub.']},{solid:false});}
  {const g=G_(at(cyl(0.22,0.26,0.6,'#C8D4E0',10),0,0.3,0),at(cyl(0.14,0.2,0.2,'#C8D4E0',10),0,0.7,0),at(cyl(0.16,0.16,0.06,'#8FA3B8',10,{ol:false}),0,0.82,0));
   place(g,35.2,21,0.3,{k:'churn',n:'the milk churn',c:['silver'],s:'tall and round',m:'metal',v:'full to the brim',u:'carry fresh milk in',e:'🥛🥫🐄',r:['I’m a tall metal can full of fresh milk from the cow.']},{solid:false});}
  ...
  {const g=new THREE.Group();g.add(at(cyl(0.035,0.035,1.7,'#B08968',5,{ol:false}),0,0.85,0));g.add(at(box(0.4,0.05,0.05,'#8FA3B8',{ol:false}),0,1.72,0));
   ...
   place(g,37.6,20.2,0.2,{k:'pitchfork',n:'the pitchfork',c:['silver','brown'],s:'long and pointy',m:'metal',v:'a bit prickly',u:'toss hay with',e:'🔱🌾😬',r:['I have three sharp teeth and I toss hay all day.']},{solid:false});}
  {const g=new THREE.Group();const tub=mk(new THREE.CylinderGeometry(0.55,0.35,0.45,4,1),'#D9534F');tub.rotation.y=Math.PI/4;tub.position.set(0,0.55,0);g.add(tub);
   ...
   place(g,44,21.5,0.6,{k:'wheelbarrow',n:'the wheelbarrow',c:['red'],s:'tippy',m:'metal',v:'always helpful',snd:'squeak',u:'carry heavy loads in',e:'🛞🪨💪',r:['I have one wheel and two handles, and I carry heavy loads.']},{solid:false});}
  {const g=G_(at(cyl(0.2,0.24,0.35,'#3DDC97',10,{w:0.02}),0,0.18,0),rot(at(cyl(0.03,0.05,0.5,'#3DDC97',6,{ol:false}),0.3,0.35,0),0,0,-0.9),rot(at(mk(new THREE.TorusGeometry(0.15,0.03,4,8,Math.PI),'#3DDC97',{ol:false}),0,0.4,0),0,Math.PI/2,0));
   place(g,47.5,17,0.3,{k:'watering can',n:'the watering can',c:['green'],s:'spouty',m:'metal',v:'generous',snd:'sprinkle',u:'water the veggies with',e:'💧🌱🚿',r:['I have a long spout and give thirsty plants a drink.']},{solid:false});}
  ...
  {const g=new THREE.Group();for(let i=0;i<6;i++){g.add(at(cone(0.06,0.25,'#FF8C1A',5,{ol:false}),i*0.35-0.9,0.06,0));g.add(at(cone(0.1,0.3,'#4CB85A',5,{ol:false}),i*0.35-0.9,0.3,0));}
   ...
   place(g,45,16.5,1,{k:'carrots',n:'the carrot patch',c:['orange','green'],s:'pointy',m:'veg',v:'crunchy and proud',snd:'crunch',u:'feed a rabbit from',e:'🥕🐰👀',r:['We grow upside down: orange underground, green leaves on top.']},{solid:false});}
  {const g=new THREE.Group();for(let i=0;i<4;i++)g.add(at(scl(sph(0.28,i%2?'#6CC05A':'#8AD65E',8,6,{w:0.02}),1,0.8,1),i*0.7-1,0.22,0));
   place(g,45,19,1,{k:'cabbages',n:'the cabbages',c:['green'],s:'round',m:'leaves',v:'leafy and layered',u:'make coleslaw from',e:'🥬🐛🥗',r:['I’m a ball of leaves inside leaves inside leaves.']},{solid:false});}
  {const g=new THREE.Group();for(let i=0;i<5;i++){g.add(at(cyl(0.05,0.06,1.7,'#6CC05A',5,{ol:false}),i*0.4-0.8,0.85,0));g.add(at(scl(sph(0.1,'#FFD23F',6,5,{ol:false}),1,2.2,1),i*0.4-0.72,1.1,0.05));}
   place(g,47.5,23,0.8,{k:'corn',n:'the corn stalks',c:['green','yellow'],s:'tall and skinny',m:'plant',v:'all ears',snd:'rustle',u:'make popcorn from',e:'🌽🍿📏',r:['I grow tall, and my yellow cobs hide in green jackets.']},{solid:false});}
  {const g=new THREE.Group();g.add(at(cyl(0.06,0.06,2.2,'#8B5E3C',5,{ol:false}),0,1.1,0));g.add(rot(at(cyl(0.05,0.05,1.8,'#8B5E3C',5,{ol:false}),0,1.6,0),0,0,Math.PI/2));
   ...
   place(g,46.5,18.2,0.4,{k:'scarecrow',n:'the scarecrow',c:['blue','straw'],s:'tall',m:'straw',v:'not very scary',u:'keep crows off the corn with',e:'🧑‍🌾🐦‍⬛🌽',r:['I stand in the field with my arms out, but the crows aren’t scared of me.']});}
  // eggs
  {const g=G_(at(cyl(0.3,0.22,0.22,'#B08968',10,{w:0.02}),0,0.11,0),rot(at(mk(new THREE.TorusGeometry(0.26,0.025,4,10,Math.PI),'#8B5E3C',{ol:false}),0,0.22,0),0,0,0));
   ...
   place(g,42.5,19.8,0.3,{k:'eggs',n:'the basket of eggs',c:['white','brown'],s:'round',m:'shell',v:'fragile',u:'make breakfast with',e:'🥚🧺🍳',r:['Handle me carefully or I’ll crack.']},{solid:false});}
  ...
  {const c=cowMesh();scene.add(c);
   ...
   W.movers.push(t=>{const a=t*0.04;const x=38+Math.cos(a)*3.5,z=21.5+Math.sin(a)*2;c.position.set(x,landH(x,z),z);c.rotation.y=-a;c.children[3].position.y=1.25-Math.max(0,Math.sin(t*0.5))*0.35;});}
  {const g=new THREE.Group();g.add(at(scl(mk(new THREE.CylinderGeometry(1.2,1.2,0.05,14),'#7A5230',{ol:false}),1,1,0.7),0,0.02,0));
   ...
   place(g,32.5,20.5,0.9,{k:'pig',n:'the muddy pig',c:['pink','brown'],s:'round',m:'skin',v:'happily filthy',snd:'oink',u:'give a belly rub',e:'🐷🟤😌',r:['I love rolling in mud, and my tail is curly.']},{solid:false});
  ...
  {const s=woolly();scene.add(s);
   ...
   W.movers.push(t=>{const a=-t*0.05+2;const x=40+Math.cos(a)*6,z=21.5+Math.sin(a)*3.2;s.position.set(x,landH(x,z),z);s.rotation.y=Math.atan2(Math.sin(a),-Math.cos(a));});}
  {const g=new THREE.Group();g.add(scl(at(sph(0.3,'#F2F2EC',8,6),0,0.5,0),0.8,0.9,1.2));g.add(at(sph(0.17,'#F2F2EC',6,5),0,0.8,0.28));
   ...
   W.movers.push(t=>{const u=(t*0.25)%4;const x=33+Math.min(u,2)*1.8-Math.max(0,u-2)*1.8,z=17.5;g.position.set(x,landH(x,z)+Math.abs(Math.sin(t*6))*0.03,z);g.rotation.y=u<2?Math.PI/2:-Math.PI/2;});}
  ...
  {const h=henMesh('#C8793C','#E8323F');scene.add(h);
   ...
   W.movers.push(t=>{[h].concat(extra).forEach((c,i)=>{const a=t*0.3+i*2.1;const x=42+Math.cos(a)*1.6+Math.sin(t*1.3+i)*0.3,z=19.8+Math.sin(a)*1.1;c.position.set(x,landH(x,z),z);c.rotation.y=-a;c.rotation.x=Math.max(0,Math.sin(t*4+i))*0.4;});});}
  {const r=henMesh('#E8323F','#FF3B3B');r.scale.setScalar(1.25);r.add(at(scl(sph(0.12,'#2B6CB0',5,4,{ol:false}),0.5,1,1.3),0,0.35,-0.25));
   r.position.set(29,landH(29,20)+1,20);r.rotation.y=Math.PI/2;scene.add(r);
...
function buildOrchard(){
  {const g=new THREE.Group();for(const s of[-1,1])g.add(at(box(0.25,3,0.25,'#8B5E3C'),1.6*s,1.5,0));
   const t=signBox('Apple Hill Orchard',3.6,0.7,'#7A2E2E','#FFFFFF');t.position.y=3;g.add(t);g.add(at(box(3.6,0.15,0.3,'#8B5E3C',{ol:false}),0,3.45,0));
   place(g,31.5,-17.5,0.3,{k:'gate',n:'the orchard gate',c:['brown','red'],s:'arched',m:'wood',v:'welcoming',u:'walk into the orchard through',e:'🚪🍎🌳',r:['Walk under me to reach the fruit trees.']},{exact:true,ry:Math.PI/2,solid:false});
   circ(31.5,-15.9,0.3);circ(31.5,-19.1,0.3);}
  const appleTree=fruitTree(35,-19,['#4FAF52','#63C35E','#3E9E4A'],'#E8323F',{k:'apple tree',n:'the apple tree',c:['green','red'],s:'round',m:'wood',v:'generous',snd:'thud (falling apple)',u:'pick a snack from',e:'🌳🍎🩺',
    r:['I grow the red fruit that keeps the doctor away.','Newton sat under a tree like me and got bonked on the head.']});
  fruitTree(39.5,-17.5,['#6CC05A','#7ED957','#58B04E'],'#C9E265',{k:'pear tree',n:'the pear tree',c:['green'],s:'round',m:'wood',v:'a bit shy',u:'pick pears from',e:'🌳🍐💡',r:['My green fruit is shaped like a lightbulb.']});
  fruitTree(38.5,-22.5,['#FFB3D1','#FF8FBF','#FFC8DF'],'#B3122E',{k:'cherry tree',n:'the cherry tree',c:['pink','red'],s:'round and fluffy',m:'wood',v:'pretty in pink',u:'pick cherries from',e:'🌸🍒🌳',r:['My tiny red fruit comes in pairs on long stems.']});
  fruitTree(42.5,-20,['#5E9E4A','#6CC05A','#4F8F3E'],'#7B3FA0',{k:'plum tree',n:'the plum tree',c:['green','purple'],s:'round',m:'wood',v:'sweet and juicy',u:'pick plums from',e:'🌳🟣😋',r:['My purple fruit dries into a wrinkly prune.']});
  ...
  {const g=new THREE.Group();g.add(at(box(2.6,0.9,1.1,'#C8935E'),0,0.45,0));for(const s of[-1,1])g.add(at(cyl(0.06,0.06,2.2,'#8B5E3C',5),1.2*s,1.1,-0.45));
   ...
   place(g,34.5,-24.5,1.4,{k:'fruit stand',n:'the fruit stand',c:['red','white'],s:'boxy',m:'wood',v:'fresh and cheerful',u:'buy fresh fruit at',e:'🍎🍐🏪',r:['I sell whatever the trees just dropped, fresh and cheap.']},{ry:Math.PI*0.8});}
  {const g=new THREE.Group();g.add(at(cyl(0.6,0.6,0.9,'#8B5E3C',12),0,0.45,0));for(let i=0;i<3;i++)g.add(at(cyl(0.62,0.62,0.06,'#2B2040',12,{ol:false}),0,0.15+i*0.3,0));
   ...
   place(g,41,-24.5,0.9,{k:'cider press',n:'the cider press',c:['brown'],s:'barrel-shaped',m:'wood',v:'squeezy',snd:'squish',u:'squeeze apples into juice with',e:'🍎➡️🧃',r:['Squeeze apples in me and out comes a sweet drink.']});}
  {const g=new THREE.Group();for(const s of[-1,1])g.add(at(box(0.08,3.2,0.08,'#B08968',{ol:false}),0.3*s,1.6,0));for(let i=0;i<7;i++)g.add(at(box(0.6,0.06,0.06,'#B08968',{ol:false}),0,0.3+i*0.42,0));g.rotation.x=-0.25;
   place(g,40.5,-16.7,0.4,{k:'ladder',n:'the tall ladder',c:['brown'],s:'tall and skinny',m:'wood',v:'always helpful',u:'reach the top fruit with',e:'🪜🍐⬆️',r:['Climb my rungs to reach the fruit at the very top.']},{solid:false});}
  {const g=new THREE.Group();for(const s of[-1,1])g.add(at(cyl(0.12,0.15,2.2,'#8B5E3C',6),1.6*s,1.1,0));
   ...
   place(g,44.5,-23,1.8,{k:'hammock',n:'the hammock',c:['orange'],s:'saggy',m:'rope',v:'lazy on purpose',snd:'creak',u:'nap in',e:'😴🌴🟧',r:['I hang between two posts and rock you to sleep.']},{ry:0.4});
  ...
  {const g=new THREE.Group();g.add(at(box(1.8,0.6,1.1,'#B08968'),0,0.75,0));for(const s of[-1,1])g.add(rot(at(cyl(0.38,0.38,0.12,'#8B5E3C',10),0.95*s,0.38,0),0,0,Math.PI/2));
   ...
   place(g,37,-26.5,1.2,{k:'cart',n:'the fruit cart',c:['brown','red'],s:'boxy',m:'wood',v:'heavy with apples',snd:'rumble',u:'wheel the harvest home in',e:'🛒🍎🍎',r:['Push me along and I’ll carry the whole harvest.']},{ry:-0.3});}
  ...
  {const g=new THREE.Group();for(const [sx,sz] of[[-1,-1],[1,-1],[-1,1],[1,1]])g.add(at(cyl(0.1,0.1,5,'#8B5E3C',5,{ol:false}),sx*0.9,2.5,sz*0.9));
   ...
   place(g,44,-16.5,1.3,{k:'water tower',n:'the water tower',c:['brown','red'],s:'tall',m:'wood',v:'towering over everyone',snd:'drip',u:'store the farm’s water in',e:'💧🗼🌳',r:['I stand on stilts holding a giant bucket of water for the whole farm.']});}
  // small things
  {const g=new THREE.Group();for(const x of[-0.4,0.4]){g.add(at(box(0.6,0.5,0.6,'#FFFFFF',{w:0.02}),x,0.45,0));g.add(at(box(0.6,0.3,0.6,'#FFFFFF',{w:0.02}),x,0.85,0));g.add(at(box(0.7,0.06,0.7,'#E8C45A',{ol:false}),x,1.03,0));}
   ...
   place(g,43.5,-26,0.6,{k:'beehives',n:'the beehives',c:['white','yellow'],s:'boxy',m:'wood',v:'buzzing with work',snd:'bzzzz',u:'collect honey from',e:'🐝🏠🍯',r:['Thousands of busy workers live in my stacked wooden boxes.']},{solid:false});
  ...
  {const g=G_(at(cyl(0.12,0.14,0.22,'#FFB627',8,{w:0.015}),0,0.11,0),at(cyl(0.13,0.13,0.06,'#B5793A',8,{ol:false}),0,0.25,0));
   const [x,z]=freeSpot(34.6,-24.1,0.1,false);g.position.set(x,landH(x,z)+0.9,z);scene.add(g);
  ...
  {const g=at(sph(0.14,'#E8323F',8,6,{w:0.02}),0,0.14,0);const w=G_(g,at(cyl(0.015,0.015,0.1,'#5B3A29',4,{ol:false}),0,0.3,0));
   place(w,36.2,-18.2,0.15,{k:'apple',n:'the fallen apple',c:['red'],s:'round',m:'fruit',v:'a little bruised',u:'pick up and polish',e:'🍎⬇️🌱',r:['I dropped off my branch and now I’m waiting in the grass.']},{solid:false});}
  {const g=G_(at(cyl(0.3,0.22,0.3,'#B08968',10,{w:0.02}),0,0.15,0));for(const [x,z] of[[-0.1,0],[0.12,0.05],[0,-0.12],[0.02,0.08]])g.add(at(sph(0.1,'#E8323F',6,5,{ol:false}),x,0.33,z));
   place(g,36.8,-21.2,0.3,{k:'basket',n:'the apple basket',c:['brown','red'],s:'round',m:'wicker',v:'full and happy',u:'fill with picked apples',e:'🧺🍎🍎',r:['Pickers fill me to the brim and carry me home.']},{solid:false});}
  {const g=G_(at(cyl(0.22,0.18,0.3,'#8FA3B8',8,{w:0.02}),0,0.15,0));for(const [x,z] of[[-0.07,0],[0.08,0.04],[0,-0.08]])g.add(at(scl(sph(0.08,'#C9E265',6,5,{ol:false}),1,1.3,1),x,0.34,z));
   place(g,40.8,-18.6,0.25,{k:'bucket',n:'the bucket of pears',c:['silver','green'],s:'round',m:'metal',v:'a bit top-heavy',u:'carry pears in',e:'🪣🍐🍐',r:['I’m a metal pail full of lightbulb-shaped fruit.']},{solid:false});}
  ...
  {const g=new THREE.Group();g.add(scl(at(sph(0.12,'#8B5E3C',6,5),0,0,0),0.9,0.9,1.3));g.add(at(scl(sph(0.08,'#E8563F',5,4,{ol:false}),1,1,0.6),0,-0.02,0.1));g.add(rot(at(cone(0.02,0.06,'#F2A541',3,{ol:false}),0,0.03,0.17),Math.PI/2,0,0));
   g.position.set(31.5,landH(31.5,-17.5)+3.62,-16.6);scene.add(g);
  ...
  {const g=G_(at(cyl(0.12,0.2,0.9,'#B8B2C8',8),0,0.45,0),at(cyl(0.5,0.3,0.18,'#B8B2C8',12),0,0.95,0),at(mk(new THREE.CylinderGeometry(0.42,0.42,0.03,12),'#6ED3EC',{ol:false}),0,1.03,0));
   place(g,33.5,-21.5,0.5,{k:'birdbath',n:'the birdbath',c:['grey','blue'],s:'round',m:'stone',v:'refreshing',snd:'splish',u:'give birds a bath',e:'🐦🛁💧',r:['Birds splash about in my shallow stone bowl.']});}
  {const g=new THREE.Group();g.add(at(scl(sph(0.08,'#C8793C',8,6,{w:0.01}),1,1,0.6),0,0.08,0));g.add(at(scl(sph(0.05,'#A7B88A',5,4,{ol:false}),2.2,0.6,0.8),0.06,0.02,0));
   g.add(at(cyl(0.01,0.01,0.06,'#A7B88A',3,{ol:false}),0.14,0.07,0));g.position.set(31.72,landH(31.5,-19.1)+1.4,-19.1);g.rotation.z=Math.PI/2;g.rotation.y=Math.PI/2;scene.add(g);
  ...
  {const g=G_(rot(at(box(0.18,0.05,0.28,'#4CB85A',{w:0.01}),-0.1,0.03,0),0,0.3,0),rot(at(box(0.18,0.05,0.28,'#4CB85A',{w:0.01}),0.12,0.03,0.05),0,-0.2,0));
   place(g,38,-25.3,0.25,{k:'gloves',n:'the gardening gloves',c:['green'],s:'flat',m:'fabric',v:'a bit muddy',u:'protect your hands in the garden',e:'🧤🌱🪴',r:['Two of us keep your hands clean while you dig.']},{solid:false});}
  {const g=G_(rot(at(box(0.04,0.35,0.05,'#E8323F',{w:0.01}),-0.04,0.05,0),0,0,0.3),rot(at(box(0.04,0.35,0.05,'#E8323F',{w:0.01}),0.04,0.05,0),0,0,-0.3),at(box(0.05,0.2,0.02,'#C8D4E0',{ol:false}),0,0.3,0));g.rotation.x=Math.PI/2;
   place(g,42,-22.5,0.2,{k:'shears',n:'the pruning shears',c:['red','silver'],s:'pointy',m:'metal',v:'snappy',snd:'snip',u:'trim branches with',e:'✂️🌿🌳',r:['Snip, snip: I give the fruit trees a haircut.']},{solid:false,y:0.03});}
}
