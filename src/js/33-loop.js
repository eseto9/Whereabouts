...
function cullTick(dt){
  ...
  for(const c of Cull.list){
    ...
    const inCam=c.near&&d<c.near&&inGame()&&P.bean&&camera.position.y<c.obj.position.y+6;
    c.obj.visible=!c.obj.userData.cmHidden&&(lit||(!inCam&&d<c.far));   // the answer is always drawn while it's being shown (cmHidden: cleared for Crowded Market)
...
function frame(now){
  ...
  if(inGame()&&P.bean){
    // your bean and the camera: the same in both games
    {const n=Math.ceil(dt/0.034);for(let i=0;i<n;i++)updatePlayer(dt/n);}updateCamera(dt);
    ...
    if(CMG.on)cmFrame(dt,ts);
    else{sendPresence();updateRemotes(dt,ts);hostTick();updateHUDTimers();updateCompass();goldTick(ts,dt);if(redT>0){redT-=dt;}}
  ...
  if(inGame()&&P.bean)shadowFollow(P.pos.x+Math.sin(P.yaw)*18,P.pos.z+Math.cos(P.yaw)*18,62);else shadowFollow(22,0,105);
