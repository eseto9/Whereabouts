/* =========================================================
   Clue generator
   ========================================================= */
const DPH={square:'around Town Square',market:'along Market Street',harbor:'down by the Harbor',hill:'up on Hillside',park:'over in the Park',sea:'out on the water'};
...
function ladderFormats(o){
  const f=[];if(o.c&&o.c.length)f.push('classic');if(o.v)f.push('vibe');if(o.snd)f.push('sound');if(o.u)f.push('use');if(o.e)f.push('emoji');if(o.r)f.push('riddle');
  return f;
}
const EASY_FMTS=['classic','vibe','use'],HARD_FMTS=['riddle','emoji','sound','use'];
// pref: a format to use (the daily hunt), or a list of formats to prefer (difficulty)
function makeLadder(o,last,pref){
  const f=ladderFormats(o);
  let opts=f.filter(x=>x!==last);if(!opts.length)opts=f;
  if(Array.isArray(pref)){const p=opts.filter(x=>pref.includes(x));if(p.length)opts=p;}
  const fmt=typeof pref==='string'&&f.includes(pref)?pref:pick(opts);
  const col=o.c&&o.c.length?o.c.join(' and '):null;
  const l1={classic:`I spy something ${col}…`,vibe:`I spy something that looks ${o.v}…`,sound:`I spy something that goes “${o.snd}”…`,use:`I spy something you could ${o.u}…`,emoji:`I spy… ${o.e}`,riddle:o.r}[fmt];
  const facts=[];
