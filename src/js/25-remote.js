...
function updateRemotes(dt,ts){
  ...
  for(const p of players()){
    ...
    seen.add(p.peer);
    const sig=pr.col+'|'+JSON.stringify(pr.fit)+'|'+pr.name;let r=Remote.get(p.peer);
    if(r&&r.sig!==sig){scene.remove(r.bean);Remote.delete(p.peer);r=null;}
    if(!r){r={bean:makeBean(safeCol(pr.col),safeFit(pr.fit),clean(pr.name)||'Bean'),sig,face:+pr.ry||0};r.bean.position.set(pr.x,+pr.y||0,+pr.z||0);Remote.set(p.peer,r);}
    ...
    animBean(b,{an:pr.an|0,em:typeof pr.em==='string'?pr.em:'',emAt:+pr.emAt||0},ts);
    b.userData.scope.scale.setScalar(pr.bi?1.6:1);
  }
